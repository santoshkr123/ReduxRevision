...
const ProductDetails =  ()=>{
    const ProductDetails = useSelector((res)=>res)
        if(!productSlice)
            return(
              <h1 className="text-6xl font-bold text-center py-12">Data not availble in redux store</h1>
            )
    ...
    return (
        <div className="bg-gray-200 min-h-screen flex justify-center items-center">
            ...
            {
                productSlice.description
            }
            </p>
            <label className="bg-blue-600 px-6 py-2 rounded text-white capitalize font-semibold">{productSlice.category}</label>
            <button className="bg-rose-600 py-2 rounded text-white font-semibold mt-6">Buy Now</button>
